...
{
    PoolID<Node> node_id = Pool::get_pool()->create<Node>();
...
void NodeFactory::add_invokable_component(PoolID<Node> _node, const fw::func_type* _func_type, const fw::iinvokable *_invokable, bool _is_operator) const
{
    ...
    component->bind_result_property( _node->props.get_id(VALUE_PROPERTY) );

    // Link arguments
    auto args = _func_type->get_args();
    for (u32_t index = 0; index < args.size(); index++)
    {
        Property* property = _node->props.get_input_at(index);
        if ( args[index].m_by_reference ) property->set_ref();  // to handle by reference function args
        component->bind_arg(index, property->id);
    }
}
