...
bool GraphNodeView::draw()
{
    ...
    fw::vec2        origin           = ImGui::GetCursorScreenPos();
    ...
    ImGui::SetCursorPos(fw::vec2(0,0));
    ...
    */
    ...
    for( Node* each_node : node_registry)
    {
        ...
        for (Node* each_successor_node : each_node->successors() )
        {
            ...
            if (each_view && each_successor_view && each_view->is_visible() && each_successor_view->is_visible() )
            {
                ...
                fw::vec2 start = each_view->get_screen_position();
                ...
                fw::vec2 end = each_successor_view->get_screen_position();
    ...
    if ( ImGui::IsWindowFocused(ImGuiFocusedFlags_ChildWindows) )
    {
        ...
        if ( dragged_property_conn )
        {
            fw::vec2 src = dragged_property_conn->get_pos();
        ...
        {
            fw::vec2 src = dragged_node_conn->get_pos();
            fw::vec2 dst = hovered_node_conn ? hovered_node_conn->get_pos() : ImGui::GetMousePos();
            ...
            for (auto& dst_property : properties)
            {
                if ( const Property * src_property = dst_property->get_input() )
                {
                    ...
                    if (src_node_view->is_visible() && dst_node_view->is_visible() )
                    {
                        ...
                        {
                            fw::vec2 src_pos = src_property_view->m_out->get_pos();
                            fw::vec2 dst_pos = dst_property_view->m_in->get_pos();
    ...
    if ( virtual_machine.is_program_running() )
    {
        ...
        {
            fw::vec2 vm_cursor_pos = view->get_screen_position();
            ...
            linePos += fw::vec2(sin(float(App::get_instance().elapsed_time()) * 12.0f ) * 4.0f, 0.f ); // wave
            float size = 20.0f;
            float width = 2.0f;
            ImColor color = ImColor(255,255,255);

            // arrow ->
            draw_list->AddLine( linePos - fw::vec2(1.f, 0.0f), linePos - fw::vec2(size, 0.0f), color, width);
            draw_list->AddLine( linePos, linePos - fw::vec2(size * 0.5f, -size * 0.5f), color, width);
            draw_list->AddLine( linePos, linePos - fw::vec2(size * 0.5f, size * 0.5f) , color, width);
        }
    }
	...
	if ( enable_edition && !isAnyNodeHovered && ImGui::BeginPopupContextWindow(k_context_menu_popup) )
	{
        ...
        */
        if (new_node)
        {
            ...
            else if ( new_node != graph->get_root() && settings.experimental_graph_autocompletion )
            {
...
bool GraphNodeView::update()
{
    ...
    return update( ImGui::GetIO().DeltaTime, Settings::get_instance().ui_node_animation_subsample_count );
}
...
void GraphNodeView::frame_views(std::vector<NodeView*>& _views) {
    ...
    fw::vec2 delta = m_visible_rect.GetSize() * 0.5f - rect.GetCenter();
...
void GraphNodeView::translate_all(fw::vec2 delta, const std::vector<NodeView*>& _views)
{
