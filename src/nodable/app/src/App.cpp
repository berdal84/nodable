...
#include <nodable/app/Condition.h>
...
void App::onUpdate()
{
    ...
    auto push_overlay_shortcuts = [](ndbl::FileView* view, Condition condition) -> void {
        for (const auto& _binded_event: fw::BindedEventManager::s_binded_events)
        {
...
bool App::open_file(const fs_path& _path)
{
    ...
    current_file(file);
    fw::EventManager::push_event(fw::EventType_file_opened);
	return true;
}
