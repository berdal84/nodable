...
namespace ndbl
{
    ...
    struct ContextMenu
    {
        ...
        std::vector<fw::Action> items;                           // All the available items
        std::vector<fw::Action> items_with_compatible_signature; // Only the items having a compatible signature (with the slot dragged)
        std::vector<fw::Action> items_matching_search;           // Only the items having a compatible signature AND matching the search_input.

        fw::Action*             draw_search_input( size_t _result_max_count ); // Return the triggered action, user has to deal with the Action.
