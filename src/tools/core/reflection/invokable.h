...
#undef CAST_5ARGS
    ...
    public:
        ...
        {
            EXPECT(_args.size() == std::tuple_size<args_t>(), "Wrong argument count!");
            return tools::invoke<return_t, args_t >(m_function_impl, _args);
        }
    ...
    public:
        ...
        {
            EXPECT(_args.size() == std::tuple_size<args_t>(), "Wrong argument count!");
            return tools::invoke_property<return_t, class_t, args_t>(reinterpret_cast<class_t *>(_instance), m_method, _args);
        };
    };
