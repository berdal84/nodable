...
void tools::Spatial_Node::set_position(const Vec2& _pos)
{
    _transform.set_position(_pos ); // PARENT_SPACE
    set_world_transform_dirty();
}

void tools::Spatial_Node::set_position(const Vec2& _pos, Space desired_space )
{
...
void tools::Spatial_Node::set_world_transform_dirty()
{
    _world_matrix_dirty = true;
    for (Spatial_Node* child : _children)
    {
        child->set_world_transform_dirty();
    }
}

void tools::Spatial_Node::update_world_matrix()
{
...
tools::Vec2 tools::Spatial_Node::position() const
{
    return _transform.position();
}

tools::Vec2 tools::Spatial_Node::position(Space space) const
{
    switch ( space )
    {
        default:
...
bool tools::Spatial_Node::add_child(tools::Spatial_Node* new_child, Spatial_Node_Flags flags)
{
...
bool tools::Spatial_Node::remove_child(tools::Spatial_Node* child, Spatial_Node_Flags flags)
{
    ASSERT(child);
...
void tools::Spatial_Node::translate(const tools::Vec2& delta)
{
