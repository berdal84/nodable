...
namespace ndbl{
    ...
    {
	public:
        using operators_vec = std::vector<const Operator*>;
        using invokable_ptr = std::shared_ptr<const iinvokable>;
        using Invokable_vec = std::vector<std::shared_ptr<const iinvokable>>;
    ...
    public:
        ...
        std::string&           serialize(std::string& _out, const func_type*, const std::vector<Property *>&)const;  // serialize a function call with arguments.
        std::string&           serialize(std::string& _out, const func_type*)const;                                  // serialize a function signature.
        ...
        // Language definition -------------------------------------------------------------------------
    public:
        invokable_ptr                   find_function(const func_type*) const;                   // Find a function by signature.
        invokable_ptr                   find_operator_fct(const func_type*) const;               // Find an operator's function by signature (casts allowed).
        invokable_ptr                   find_operator_fct_exact(const func_type*) const;         // Find an operator's function by signature (strict mode, no cast allowed).
        ...
        std::string&                    to_string(std::string& /*out*/, const type&)const;              // Convert a type to string (by ref).
    ...
    private:
        ...
        invokable_ptr                   find_operator_fct_fallback(const func_type*) const;      // Find a fallback operator function for a given signature (allows cast).
