...
namespace ndbl
{
    ...
    [[deprecated]] bool             language_is_initialized(); // TODO: this should not exist, user must know if he already initialized a parser. The problem comes from the fact some functions in this struct are used by code that do not need a parser.
    Language&                       parser();
}
