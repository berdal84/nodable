...
namespace ndbl
{
    ...
    enum ScopeFlags_
    {
        ...
        ScopeFlags_AS_PRIMARY_CHILD        = 1 << 4
    ...
    public:
        ...
        static void                    change_children_scope(ASTScope* source, ASTScope* target);
        static void                    change_scope(ASTNode *node, ASTScope* desired_scope, ScopeFlags = ScopeFlags_NONE);

    private:
        bool                           _append(ASTNode *, ScopeFlags = ScopeFlags_NONE);
        ...
        bool is_primary(ASTNode *node) const;
    };
}
