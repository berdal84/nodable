...
ASTGraph::ASTGraph(ASTNodeFactory* factory)
: m_factory(factory)
{}
...
void ASTGraph::clear()
{
    ...
    {
        ASTNode* node = m_node_registry[0];
...
UpdateResult ASTGraph::update()
{
    ...
    {
        nodeIndex--;
        ASTNode* node = m_node_registry.at(nodeIndex);
...
void ASTGraph::add(ASTNode* _node)
{
...
void ASTGraph::remove(ASTNode* _node)
{
...
ASTVariableNode* ASTGraph::create_variable(const TypeDescriptor *_type, const std::string& _name, ASTScope* _scope)
{
    ASTVariableNode* node = m_factory->create_variable(_type, _name, _scope);
    add(node);
	return node;
}

ASTFunctionNode* ASTGraph::create_function(const FunctionDescriptor* _type)
{
    ASTFunctionNode* node = m_factory->create_function(_type, ASTNodeType_FUNCTION);
    add(node);
    return node;
}

ASTFunctionNode* ASTGraph::create_operator(const FunctionDescriptor* _type)
{
    ASTFunctionNode* node = m_factory->create_function(_type, ASTNodeType_OPERATOR);
    add(node);
    return node;
}

void ASTGraph::destroy(ASTNode* node)
{
    if( node == nullptr )
    {
        return;
    ...
    {
        ASTVariableNode* node_variable = cast<ASTVariableNode>(node);
        if ( ASTScopeInterface* scope = node_variable->get_scope() )
        {
            scope->remove_variable(node_variable);
        }
    }
    else if ( auto* scope = node->get_component<ASTScope>() )
    {
...
bool ASTGraph::is_empty() const
{
    return m_root == nullptr;
}

DirectedEdge* ASTGraph::connect_or_merge(Slot&_out, Slot& _in )
{
    // Guards
...
//    EXPECT( type::is_implicitly_convertible( out_type, in_type ), "dependency type should be implicitly convertible to dependent type");
    ...
    if (_out.node()->type() == ASTNodeType_LITERAL )
        if (_in.node()->type() != ASTNodeType_VARIABLE )
        {
            in_prop->digest( out_prop );
            destroy(_out.node());
...
void ASTGraph::remove(DirectedEdge edge)
{
...
DirectedEdge* ASTGraph::connect_to_variable(Slot& _out, ASTVariableNode& _variable )
{
    // Guards
...
DirectedEdge* ASTGraph::connect(Slot& _first, Slot& _second, ConnectFlags _flags)
{
    ...
    if (_flags & ConnectFlag_ALLOW_SIDE_EFFECTS )
    {
        switch ( type )
        {
            case SlotFlag_TYPE_HIERARCHICAL:
            {
                ...
                ASTNode* parent    = _first.node();  static_assert(SlotFlag_CHILD & SlotFlag_ORDER_FIRST);
                ASTNode* new_child = _second.node(); static_assert(SlotFlag_PARENT & SlotFlag_ORDER_SECOND);
                ASSERT( parent->has_component<ASTScope>())
                ...
                {
                    ASTNode* previous_child = parent->rchildren().at(1);
                    ...
                    //             - new child <-<-<-<-<-<-<-<
                    //
                    if ( previous_child->has_component<ASTScope>() )
                    {
                        auto previous_child_scope = previous_child->get_component<ASTScope>();
                        for (ASTNode* each_instr : previous_child_scope->get_last_instructions_rec() )
                        {
            ...
            {
                ASTNode& prev_node = *_first.node(); static_assert(SlotFlag_NEXT & SlotFlag_ORDER_FIRST );
                ASTNode& next_node = *_second.node(); static_assert(SlotFlag_PREV & SlotFlag_ORDER_SECOND );

                // If previous node is a scope, connects next_node as child
                if ( prev_node.has_component<ASTScope>() )
                {
                    connect(
                ...
                else if ( ASTNode* prev_parent_node = prev_node.find_parent() )
                {
                    connect(
                ...
                {
                    ASTNode* current_prev_node_sibling = prev_node.successors()[0];
...
void ASTGraph::disconnect(const DirectedEdge& _edge, ConnectFlags flags)
{
    ...
    switch ( type )
    {
        case SlotFlag_TYPE_CODEFLOW:
        {
            ...
            ASTNode* next = _edge.head->node();
            ASTNode* next_parent = next->find_parent();
            if ( flags & ConnectFlag_ALLOW_SIDE_EFFECTS && next_parent )
            {
                while ( next && next_parent == next->find_parent() )
                {
                    ...
                    std::vector<ASTNode*> successors = next->successors();
...
ASTNode* ASTGraph::create_scope()
{
    ASTNode* scopeNode = m_factory->create_scope();
    add(scopeNode);
    return scopeNode;
}

ASTConditionalNode* ASTGraph::create_cond_struct()
{
    ASTConditionalNode* condStructNode = m_factory->create_cond_struct();
    add(condStructNode);
    return condStructNode;
}

ASTForLoopNode* ASTGraph::create_for_loop()
{
    ASTForLoopNode* for_loop = m_factory->create_for_loop();
    add(for_loop);
    return for_loop;
}

ASTWhileLoopNode* ASTGraph::create_while_loop()
{
    ASTWhileLoopNode* while_loop = m_factory->create_while_loop();
    add(while_loop);
    return while_loop;
}

ASTNode* ASTGraph::create_root()
{
    ASTNode* node = m_factory->create_program();
    add(node);
    m_root = node;
    return node;
}

ASTNode* ASTGraph::create_node()
{
...
ASTLiteralNode* ASTGraph::create_literal(const TypeDescriptor *_type)
{
    ASTLiteralNode* node = m_factory->create_literal(_type);
    add(node);
    return node;
}

ASTNode* ASTGraph::create_node(CreateNodeType _type, const FunctionDescriptor* _signature )
{
    switch ( _type )
    {
        /*
         ...
         */
        case CreateASTNodeType_BLOCK_CONDITION:  return create_cond_struct();
        case CreateASTNodeType_BLOCK_FOR_LOOP:   return create_for_loop();
        case CreateASTNodeType_BLOCK_WHILE_LOOP: return create_while_loop();
        case CreateASTNodeType_BLOCK_SCOPE:      return create_scope();
        case CreateASTNodeType_BLOCK_PROGRAM:    clear(); return create_root();

        case CreateASTNodeType_VARIABLE_BOOLEAN: return create_variable_decl<bool>();
        case CreateASTNodeType_VARIABLE_DOUBLE:  return create_variable_decl<double>();
        case CreateASTNodeType_VARIABLE_INTEGER: return create_variable_decl<int>();
        case CreateASTNodeType_VARIABLE_STRING:  return create_variable_decl<std::string>();

        case CreateASTNodeType_LITERAL_BOOLEAN:  return create_literal<bool>();
        case CreateASTNodeType_LITERAL_DOUBLE:   return create_literal<double>();
        case CreateASTNodeType_LITERAL_INTEGER:  return create_literal<int>();
        case CreateASTNodeType_LITERAL_STRING:   return create_literal<std::string>();
...
ASTVariableRefNode* ASTGraph::create_variable_ref()
{
    ASTVariableRefNode* node = m_factory->create_variable_ref();
    add(node);
    return node;
}

ASTVariableNode* ASTGraph::create_variable_decl(const TypeDescriptor* _type, const char*  _name, ASTScope*  _scope)
{
    if( !_scope)
    {
        _scope = get_root()->get_component<ASTScope>();
    }

    // Create variable
    ASTVariableNode* var_node = create_variable(_type, _name, _scope );
    var_node->set_flags(VariableFlag_DECLARED); // yes, when created from the graph view, variables can be undeclared (== no scope).
    ASTToken token(TokenType::keyword_operator, " = ");
...
void ASTGraph::set_view(ndbl::GraphView* view)
{
    ASSERT(view != nullptr);
    m_view = view;
}
