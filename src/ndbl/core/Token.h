...
namespace ndbl
{
	...
	struct Token
	{
        size_t      m_index = 0; // in parent Token_Ribbon
        Token_Type  m_type;
        ...
        Token(Token_Type type, const std::string& word )
        : m_type( type )
        {
            word_replace( word.c_str() );
        }

        Token(Token_Type type, const char* const word)
        : m_type(type)
        {
