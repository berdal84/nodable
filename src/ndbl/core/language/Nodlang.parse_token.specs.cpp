...
TEST_F(parse_token, atomic_expression_if)
{
    std::string buffer{"if"};
    Token token = app.language.parse_token(buffer);
    EXPECT_EQ(token.m_type, Token_t::keyword_if);
    EXPECT_EQ(token.buffer_to_string(), "if");
}

TEST_F(parse_token, atomic_expression_else)
{
...
TEST_F(parse_token, atomic_expression_for)
{
