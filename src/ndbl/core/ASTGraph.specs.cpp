...
typedef ::testing::Core ASTGraph_;

TEST_F(ASTGraph_, connect)
{
    // Prepare
    ASTGraph* graph = app.get_graph();
...
TEST_F(ASTGraph_, clear)
{
    ASTGraph* graph = app.get_graph();
    EXPECT_TRUE( graph->get_node_registry().empty() );
    EXPECT_TRUE( graph->get_edge_registry().empty() );

    ASTVariableNode*     variable  = graph->create_variable(type::get<int>(), "var", nullptr);
...
TEST_F(ASTGraph_, create_and_delete_relations)
{
    // prepare
    ASTGraph* graph = app.get_graph();
