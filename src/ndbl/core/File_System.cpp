...
Path Path::get_executable_path()
{
    ...
    if (length > 0)
    {
        ...
        if ( wai_getExecutablePath(path, length, &dirname_length) )
        {
            path[length] = '\0';
            result = path;
            NDBL_DEBUG_LOG(Verbosity_Diagnostic, "system", "executable path: %s\n", result.c_str() );
            NDBL_DEBUG_LOG(Verbosity_Diagnostic, "system", "  dirname: %s\n", result.parent_path().c_str() );
            NDBL_DEBUG_LOG(Verbosity_Diagnostic, "system", "  basename: %s\n", result.filename().c_str() );
        }
        else
        {
            NDBL_LOG(Verbosity_Error, "system", "Unable to get executable path\n");
        }
    }
    else
    {
        NDBL_LOG(Verbosity_Warning, "system", "Unable to get executable path!\n");
    }
    return result;
#else
