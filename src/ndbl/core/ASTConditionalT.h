...
namespace ndbl
{
    ...
    template<size_t BRANCH_COUNT>
    Slot& ASTConditionalT<BRANCH_COUNT>::get_condition_slot(Branch _branch )
    {
        return const_cast<Slot&>( const_cast<const ASTConditionalT<BRANCH_COUNT>*>(this)->get_condition_slot(_branch) );
    }

    template<size_t BRANCH_COUNT>
    const Slot& ASTConditionalT<BRANCH_COUNT>::get_condition_slot(Branch _branch ) const
    {
    ...
    template<size_t BRANCH_COUNT>
    ASTNode* ASTConditionalT<BRANCH_COUNT>::get_condition(Branch _branch ) const
    {
    ...
    template<size_t BRANCH_COUNT>
    void ASTConditionalT<BRANCH_COUNT>::init(ASTNode* node)
    {
        ...
        node->add_component( new ASTScope() );
        ...
        auto condition_property = node->add_prop<ASTNode*>(CONDITION_PROPERTY);
    ...
    template<size_t BRANCH_COUNT>
    const Slot& ASTConditionalT<BRANCH_COUNT>::get_child_slot_at(Branch _branch ) const
    {
    ...
    template<size_t BRANCH_COUNT>
    ASTScope* ASTConditionalT<BRANCH_COUNT>::get_scope_at(Branch _branch ) const
    {
        ASSERT(_branch < BRANCH_COUNT )
        if ( Slot* adjacent_slot = m_child_slot[_branch]->first_adjacent() )
        {
            return adjacent_slot->node()->get_component<ASTScope>();
        }
        return {};
    }

    template<size_t BRANCH_COUNT>
    Slot& ASTConditionalT<BRANCH_COUNT>::get_child_slot_at(Branch _branch )
    {
        return const_cast<Slot&>(const_cast<const ASTConditionalT<BRANCH_COUNT>*>(this)->get_child_slot_at(_branch));
    }
}
