...
#define VERIFY_TASKMANAGER_IS_INITIALIZED() VERIFY(g_task_manager != nullptr, "Task_Manager is not initialized, did you call task_manager_init() ?")
...
Task_Manager* task_manager_init(const Task_Manager::Config& config)
{
    ...
    VERIFY(config.max_capacity >= config.reserve_size, "[init_task_manager] can't reserve more space than capacity!" );
    g_task_manager = bdc::memory_new<Task_Manager>();
    g_task_manager->tasks.reserve(config.reserve_size );
    return g_task_manager;
}

void task_manager_shutdown()
{
...
void task_manager_schedule_task(const std::function<void(void)>& function, u64_t delay_in_ms )
{
    VERIFY_TASKMANAGER_IS_INITIALIZED();
