...
namespace ndbl
{
	...
	struct ASTToken
	{
        ...
        TokenType     m_type             = TokenType::null;
        ...
        ASTToken(TokenType _type, const char* const _word)
            : ASTToken(_type)
        {
        ...
        {
            VERIFY(other.m_is_buffer_owned == false, "Can't new_node a Token from an owned const char*")
        }
        ...
        bool        is_null() const { return m_type == TokenType::null; }
        void        word_replace(const char* new_word);
        void        suffix_append(const char* str);

        static const ASTToken s_null; // To act as null Token
