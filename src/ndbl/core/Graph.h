...
#include "tools/core/ComponentCollection.h"
...
namespace ndbl
{
    ...
    enum GraphFlag_
    {
        ...
        GraphFlag_TRY_TO_MAINTAIN_CODEFLOW      = 1 << 1,
    };
	...
	public:
        ...
        tools::ComponentCollection<Graph>*       components()       { return &m_component_collection; }
        const tools::ComponentCollection<Graph>* components() const { return &m_component_collection; }
        ...
        void                     trash(ASTNode* node) { trash_ex(node, false); }
        void                     trash_ex(ASTNode* node, bool with_inputs );
        void                     trash(ASTScope*);
    ...
    private:
        ...
        void                   reset_node(ASTNode*, GraphFlags = GraphFlag_NONE);
        ...
        size_t                 empty_node_trash();

        const ASTNodeFactory* m_factory  = nullptr;
        NodeRegistry          m_node_registry;
        std::queue<ASTNode*>  m_node_trash_queue;
        EdgeRegistry          m_edge_registry;
        tools::ComponentCollection<Graph> m_component_collection;
    };
}