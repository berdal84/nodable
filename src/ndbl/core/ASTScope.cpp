...
REFLECT_STATIC_INIT
{
    type::Initializer<ASTScope>("Scope");
}
...
ASTVariableNode* ASTScope::find_variable(const std::string &_identifier)
{
    ...
    ASTNode* parent = m_owner->find_parent();
    ASSERT(parent != m_owner);
    if ( !parent )
        return nullptr;
    auto* scope = parent->get_component<ASTScope>();
    if ( !scope )
        return nullptr;
    ASSERT(scope != this);
...
void ASTScope::add_variable(ASTVariableNode* _variable)
{
...
std::vector<ASTNode*>& ASTScope::get_last_instructions_rec(std::vector<ASTNode*>& _out)
{
    std::vector<ASTNode*> children = m_owner->children();
    ...
    {
        ASSERT(children[i])
        if ( ASTScope* scope = children[i]->get_component<ASTScope>() )
        {
...
void ASTScope::remove_variable(ASTVariableNode* _variable)
{
...
size_t ASTScope::remove_all_variables()
{
    size_t count = m_variables.size();
    for(ASTVariableNode* each_variable : m_variables)
    {
...
std::vector<ASTNode*> ASTScope::get_last_instructions_rec()
{
