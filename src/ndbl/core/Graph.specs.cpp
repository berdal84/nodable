...
TEST_F(Graph_, disconnect)
{
    ...
    EXPECT_EQ(app.graph.get_edge_registry().size(), 0);
    DirectedEdge& edge = *app.graph.connect_or_merge( node_1->get_slot_at( slot_1 ), node_2->get_slot_at( slot_2 ) );
    EXPECT_EQ(app.graph.get_edge_registry().size(), 1);

    // Act
    app.graph.disconnect(edge, ConnectFlag_ALLOW_SIDE_EFFECTS );
...
TEST_F(Graph_, clear)
{
    EXPECT_TRUE( app.graph.get_node_registry().empty() );
    EXPECT_TRUE( app.graph.get_edge_registry().empty() );

    auto       variable     = app.graph.create_variable( type::get<int>(), "var", PoolID<Scope>::null);
    func_type* fct_type     = func_type_builder<int(int, int)>::with_id("+");
    auto       operator_fct = app.language.find_operator_fct_exact(fct_type);

    EXPECT_TRUE(operator_fct.get() != nullptr);
    auto operator_node = app.graph.create_operator(operator_fct.get());
    ...
    EXPECT_FALSE( app.graph.get_node_registry().empty() );
    EXPECT_FALSE( app.graph.get_edge_registry().empty() );
...
{
    // prepare
    auto& edges = app.graph.get_edge_registry();
    EXPECT_EQ(edges.size(), 0);
    auto node_1 = app.graph.create_scope();
    EXPECT_EQ(edges.size(), 0);
    auto node_2 = app.graph.create_node();
    ...
    DirectedEdge* edge_1 = app.graph.connect(
