    ...
    , m_spatial_node(shape->spatial_node())
{
    ...
    CONNECT(ComponentFor::on_owner_init, &ASTNodeView::on_owner_init, this );
}
...
void ASTNodeView::arrange_recursively(bool _smoothly)
{
    ...
    if (m_node->has_internal_scope() )
        for ( ASTNode* node : m_node->internal_scope()->primary_child() )
            if ( ASTNodeView* node_view = node->components()->get<ASTNodeView>() )
...
bool ASTNodeView::draw_as_properties_panel(ASTNodeView *_view, bool* _show_advanced)
{
    ...
    for (ComponentFor<ASTNode>* component : *node->components() )
    {
        ...
        if( ImGui::TreeNode(component, "Component %s", name ) )
        {
...
void ASTNodeView::set_children_visible(bool visible, bool recursively)
{
    ...
    for(ASTScope* _scope : scopes)
    {
        ...
        {
            auto* child_state_component = _child_node->components()->get<ViewStateComponent>();
            VERIFY(child_state_component, "ViewStateComponent is required on this _child_node");
            child_state_component->data()->set_visible(visible );
        }
    }
}
