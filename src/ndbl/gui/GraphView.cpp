...
GraphView::~GraphView()
{
    DISCONNECT(_m_graph->node_added    , this);
    DISCONNECT(_m_graph->changed , this);
    DISCONNECT(_m_graph->completed  , this);
}
...
void GraphView::_on_add_node(ASTNode* node)
{
    ...
    auto* view_state_component = node->components()->create<ViewStateComponent>();
    node->components()->create<ASTNodeView>( view_state_component->data(), shape_component->data() );
    ...
    if ( node->has_internal_scope() )
    {
        ...
        scope_view->init( internal_scope, _spatial_component->data(), view_state_component->data() );
        ...
        for ( ASTScope* sub_scope : internal_scope->partition() )
        {
            ...
            sub_scope_view->init(sub_scope, _spatial_component->data(), view_state_component->data() );
        }
    }
}
...
bool GraphView::draw(float dt)
{
    ...
    for (ASTNode* node : graph()->nodes()  )
    {
        ...
        if (interpreter->is_debugging())
            if (interpreter->is_next_node(nodeview->entity()->components()->get<ASTNode>() ))
...
void GraphView::_update(float dt)
{
    ...
    // TODO: store PhysicsComponent contigously, and iterate over all of them.
    //       this requires to store the components on a per Graph basis.
    for ( ASTNode* node : graph()->nodes() ) node->get_component<PhysicsComponent>()->apply_constraints(dt);
    for ( ASTNode* node : graph()->nodes() ) node->get_component<PhysicsComponent>()->apply_forces(dt);

    LOG_VERBOSE("GraphView", "Constraints updated.\n");

    // NodeViews
    for (ASTNode* node : graph()->nodes() )
        node->components()->get<ASTNodeView>()->update(dt);
