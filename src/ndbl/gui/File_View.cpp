...
void File_View::init(File& _file)
{
    ...
    m_graph_view = componentbag_get<Graph_View>(&m_file->graph()->component_bag);
    VERIFY( m_graph_view, "A Graph_View component is required by File_View" );
}
...
void File_View::draw(float dt)
{
    ...
    ImGui::BeginChild("File_View", ImGui::GetContentRegionAvail(), false, 0);
    {
        ...
        ImGui::BeginChild("text_editor", text_editor_size, false);
        {
            ...
            draw_overlay(m_text_overlay_window_name.c_str(), m_overlay_data[Overlay_Type_TEXT], overlay_rect, Vec2(0, 1));
        ...
        ImGui::BeginChild("graph", graph_editor_size, false, flags);
        {
            ...
            draw_overlay(m_graph_overlay_window_name.c_str(), m_overlay_data[Overlay_Type_GRAPH], overlay_rect, Vec2(1, 1));
            ImGuiEx::DebugRect( overlay_rect.min, overlay_rect.max, IM_COL32( 255, 255, 0, 127 ) );
...
std::string File_View::get_text( Isolation mode )const
{
...
void File_View::set_text(const std::string& text, Isolation mode)
{
...
void File_View::set_undo_buffer(TextEditor::IExternalUndoBuffer* _buffer ) {
	this->m_text_editor.SetExternalUndoBuffer(_buffer);
}

void File_View::draw_info_panel() const
{
    // Basic inFormation
...
void File_View::experimental_clipboard_auto_paste(bool _enable)
{
...
void File_View::draw_overlay(const char* title, const std::vector<Overlay_Data>& overlay_data, const Rect& rect, const Vec2& position)
{
    ...
    if (ImGui::Begin(title, &show, flags) )
    {
        ...
        std::for_each(overlay_data.begin(), overlay_data.end(), [](const Overlay_Data& _data) {
...
void File_View::clear_overlay()
{
    for(auto& vec : m_overlay_data )
        vec.clear();
}

void File_View::push_overlay(Overlay_Data overlay_data, Overlay_Type overlay_type)
{
...
void File_View::refresh_overlay(Condition _condition )
{
    for (const IAction* _action: get_action_manager()->get_actions())
    {
        if( ( _action->userdata & _condition) == _condition && (_action->userdata & Condition_HIGHLIGHTED) )
        {
            ...
            Overlay_Type_ overlay_type = _action->userdata & Condition_HIGHLIGHTED_IN_TEXT_EDITOR ? Overlay_Type_TEXT
                                                                                                 : Overlay_Type_GRAPH;
