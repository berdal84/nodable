...
namespace ndbl
{
    ...
    struct Node_Search_Input
    {
        ...
        std::vector<Action>     items;                                  // All the available items
    ...
    Action*  nodeview_contextmenu_draw_search_input(Node_Search_Input*, Node_Slot_View* dragged_slotview, size_t result_max_count ); // Return the triggered action, user has to deal with the Action.
}