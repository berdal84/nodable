...
namespace ndbl
{
    ...
    Language& language_init()
    {
        ...
        Language* parser = bdc::memory_new<Language>();

        // A.1. Define the parser
        //-------------------------
        array_init(parser->definition.chars);

        array_append(parser->definition.chars, {
        ...
        array_init(parser->definition.keywords);
        array_append(parser->definition.keywords, {
        ...
        array_init(parser->definition.types);
        array_append(parser->definition.types, {
        ...
        array_init(parser->definition.operators);
        array_append(parser->definition.operators, {
        ...
        // A.2. Create indexes
        //---------------------
        for( auto& character : parser->definition.chars)
        {
            hashmap_add(parser->token_type_by_single_char, character.id        , character.token_type);
            hashmap_add(parser->single_char_by_keyword   , character.token_type, character.id        );
        }

        for( auto& keyword : parser->definition.keywords)
        {
            hashmap_add(parser->token_type_by_keyword, string_hash(keyword.id).hash, keyword.token_type);
            hashmap_add(parser->keyword_by_token_type, keyword.token_type          , keyword.id        );
        }

        for( auto& type : parser->definition.types)
        {
            hashmap_add(parser->keyword_by_token_type         , type.token_type                       , type.id               );
            hashmap_add(parser->keyword_by_type_id            , type.type_descriptor->id.hash_code()  , type.id               );
            hashmap_add(parser->token_type_by_keyword         , string_hash(type.id).hash             , type.token_type       );
            hashmap_add(parser->token_type_by_type_id         , type.type_descriptor->id.hash_code()  , type.token_type       );
            hashmap_add(parser->type_descriptor_by_token_type , type.token_type                       , type.type_descriptor  );
        }

        array_init(parser->operators);
        for( const Operator& operator_ : parser->definition.operators)
        {
            // TODO: implement a hash function instead of checking all each time!
            for(const auto& existing_op : parser->operators)
            {
                VERIFY(existing_op != operator_, "The same operator already exists!");
            }
            array_append(parser->operators, operator_);
        }
    ...
    Language& parser()
    {
        VERIFY(g_language, "No parser found, did you call init_language?");
        return *g_language;
    }
    ...
    Node* lang_parse_if_block(Language& lang, Scope* parent_scope, Node_Slot* flow_out)
    {
        ...
        // if's block
        Node* if_block = lang_parse_atomic_code_block( lang,  if_node->internal_scope, if_node->component.branching.branch_out(Branch_TRUE) );
        if( if_block )
        {
