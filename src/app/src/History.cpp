...
void History::push_command(s_ptr<ICommand> _cmd, bool _from_text_editor)
{
...
void History::undo()
{
	if ( !m_past.empty() )
	{
        s_ptr<ICommand> command_to_undo = m_past.front();
...
void History::redo()
{
	if ( !m_future.empty() )
	{
        s_ptr<ICommand> command_to_redo = m_future.front();
