...
TEST(Member, Type_Boolean)
{
    ...
    m.set(true);
    EXPECT_TRUE((bool)m);
    EXPECT_EQ(m.get_type()->get_typename(), Typename::Boolean);
...
TEST(Member, Type_String)
{
    ...
    EXPECT_EQ(m.get_type()->get_typename(), Typename::String);
    EXPECT_TRUE(m.is_defined());
}
...
TEST(Member, Type_Double)
{
    ...
    EXPECT_EQ(m.get_type()->get_typename(), Typename::Double);
    EXPECT_TRUE(m.is_defined());
}
...
TEST(Member, Modify_by_reference_using_a_reference)
{
    ...
    EXPECT_EQ(m1.get_type()->get_typename(), Typename::Double);
